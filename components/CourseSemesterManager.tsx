          ...
          <div className="min-w-0 space-y-5">
            ...
            {selected && <>
              ...
              {selected.courses.map((course) => <article key={course.id} className="space-y-4 rounded-2xl border border-black/10 bg-white/60 p-5 dark:border-white/10 dark:bg-white/5">
                <a href={`/workspace?tab=coverage&course=${encodeURIComponent(course.id)}`} className="inline-block text-xs font-bold text-[var(--hub-accent)] underline underline-offset-4">Open exam coverage map →</a>

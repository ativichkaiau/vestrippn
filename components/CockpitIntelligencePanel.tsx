...
const HUB_CONFIG: Record<IntelligenceHub, HubConfig> = {
  ...
  academics: {
    ...
    prompt: 'Ask for a study plan, next exam target, or clinical case drill.',
    caution: 'AI-assisted academic output. Verify against course material and trusted references before use.',
    actions: [
      { label: 'Generate study plan', detail: 'Build around HNS-2, Anki load, clinical cases, and post-HMS-2 consolidation.', outcome: 'Study-plan response layout prepared.' },
      { label: 'Review next exam target', detail: 'Focus the cockpit on HNS-2 while keeping HMS-2 marked complete.', outcome: 'Exam-target brief layout prepared.' },
...
const STATUS_COPY: Record<AssistantStatus, { label: string; detail: string }> = {
  empty: { label: 'No context selected yet.', detail: 'Choose a suggested action or draft a prompt to ask Claude.' },
  thinking: { label: 'Claude is thinking...', detail: 'Generating a response from the current hub context.' },
  ready: { label: 'Response ready for review.', detail: 'Generated by Claude. Verify before acting on it.' },
  ...
  'task-created': { label: 'Task created.', detail: 'Preview state only. Planner writes are not wired yet.' },
};

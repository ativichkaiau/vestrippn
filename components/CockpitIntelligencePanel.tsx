...
const STATUS_COPY: Record<AssistantStatus, { label: string; detail: string }> = {
  empty: { label: 'No context selected yet.', detail: 'Choose a suggested action or draft a prompt to ask ChatGPT.' },
  thinking: { label: 'ChatGPT is thinking...', detail: 'Generating a response from the current hub context.' },
  ready: { label: 'Response ready for review.', detail: 'Generated by ChatGPT. Verify before acting on it.' },
  error: { label: 'Could not generate a response.', detail: 'The assistant call failed. Check the connection and retry.' },
  saved: { label: 'Saved to Archive.', detail: 'Preview state only — Archive writes are not wired yet.' },
  'task-created': { label: 'Task created.', detail: 'Added to your planner. Open Tasks to see it.' },
};
...
export default function CockpitIntelligencePanel({
  ...
  const activeTitle = selectedAction?.label || 'Assistant response';
  ...
  const previewAction = async (action?: SuggestedAction) => {
    const active = action || selectedAction || config.actions[0];
    ...
    try {
      ...
      if (!text.trim()) throw new Error('assistant_empty');
    } catch (error) {
  ...
  const runPrompt = () => {
    ...
    void previewAction({
      label: 'Custom prompt',
      detail: prompt.trim(),
      outcome: 'Custom response generated.',
    });
  };
  ...
  const confirm = async () => {
    ...
    if (intent === 'task') {
      // Turn the response into a real planner task (first meaningful line as the
      // title, hub as the category). Requires a signed-in session.
      const title = (response.split('\n').map((l) => l.trim()).find(Boolean) || activeTitle).slice(0, 140);
      try {
        await addTask(title, hub.toUpperCase());
        setStatus('task-created');
      } catch {
        setErrorDetail('Could not create the task — make sure you are signed in.');
        setStatus('error');
      }
      return;
    }

    // Archive save is still a preview (no archive-notes store yet).
    setStatus('saved');
  };
        ...
        <motion.div variants={motionOff ? undefined : staggerContainer(0.04, 0.06)} className="space-y-4">
          <motion.div variants={motionOff ? undefined : fadeUp}>
            ...
            <div className="mt-1 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              ...
              <span className="inline-flex w-fit items-center gap-1.5 rounded-full border border-black/5 bg-black/[0.035] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-neutral-500 dark:border-white/10 dark:bg-white/[0.06] dark:text-neutral-400">
        ...
        >
          ...
          <div className="min-h-[260px] py-4">
            ...
            {(status === 'ready' || status === 'saved' || status === 'task-created') && (
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-2 max-h-[420px] overflow-y-auto whitespace-pre-wrap text-sm font-medium leading-6 text-white/80">
                    {response || activeOutcome}
                  </p>
                </div>
              </div>
            )}
            ...
            {status === 'error' && (
              <div className="rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4">
                ...
                <button
                  type="button"
                  onClick={() => void previewAction(selectedAction || config.actions[0])}
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.045] p-3 shadow-[inset_1px_1px_0_rgba(255,255,255,0.06)]">
            ...
            <p className="mt-1 text-xs font-medium leading-5 text-white/55">
              Generated live by ChatGPT from the hub context shown. {config.caution}
            </p>
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <button type="button" onClick={() => void previewAction(selectedAction || config.actions[0])} className="rounded-full bg-white px-3 py-2 text-[10px] font-black uppercase tracking-widest text-neutral-950 transition active:scale-95">
              Regenerate
            </button>
          </div>
        </motion.div>
        ...
        <div className="fixed inset-0 z-[280] flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm" data-no-typewriter>
          ...
          >
            ...
            <p className="mt-2 text-sm font-medium leading-6 text-neutral-500 dark:text-neutral-400">
              {confirmIntent === 'task'
                ? 'This adds a task to your planner from the response above. You must be signed in.'
                : 'Save-to-Archive is still a preview — confirming only shows the success state; no data is saved yet.'}
            </p>
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button type="button" onClick={() => void confirm()} className="rounded-full bg-neutral-950 px-4 py-2 text-[11px] font-black uppercase tracking-widest text-white transition active:scale-95 dark:bg-white dark:text-black">
                {confirmIntent === 'task' ? 'Create task' : 'Confirm preview'}
              </button>
            </div>
          </motion.div>
        </div>

      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden bg-base">
        ...
        <main className="flex-1 flex flex-col gap-6 p-4 md:p-6 overflow-y-auto overflow-x-hidden">
          ...
          <div className="flex justify-between items-end shrink-0 mb-2">
            <div>
              <h1 className="font-barlow text-[28px] text-textPri font-bold uppercase tracking-wide leading-none">Meta-Analysis War Room</h1>
              <p className="text-[13px] text-textSec mt-1">Systematic Review Engine & Covidence Protocol</p>
            </div>
            <div className="flex gap-3">
              <div className="text-[11px] font-mono text-accentAmber border border-accentAmber/30 bg-accentAmber/10 px-3 py-1 rounded flex items-center gap-2">
          ...
          {/* OMNI-SEARCH FETCHER */}
          <div className="bg-surface border border-borderline rounded-lg p-5 shadow-sm shrink-0 hover:border-accentCyan/40 transition-colors">
            <div className="font-barlow font-semibold text-[13px] uppercase tracking-wide text-textSec mb-3 flex items-center gap-2">
            ...
            <div className="flex flex-col md:flex-row gap-3">
              <input 
                type="text" 
                placeholder='Enter search string (e.g., "vaginal estrogen" AND "urinary tract infection")'
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleSearch('pubmed')}
                className="flex-1 bg-base border border-borderline rounded px-4 py-2.5 text-[13px] text-textPri outline-none focus:border-accentCyan/50 transition-colors font-mono placeholder:text-textMuted"
              />
              
              <div className="flex gap-2 shrink-0">
                <button 
                  onClick={() => handleSearch('pubmed')}
                  className="bg-base border border-borderline hover:border-accentCyan hover:text-accentCyan hover:bg-accentCyan/5 text-textSec px-4 py-2.5 rounded text-[11px] font-bold uppercase tracking-wider transition-all"
                >
                  PubMed
                </button>
                <button 

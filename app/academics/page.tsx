  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden bg-base">
        ...
        {/* SIDEBAR */}
        <aside className={`w-full md:w-[220px] border-b md:border-b-0 md:border-r border-borderline flex flex-row md:flex-col justify-between p-4 md:p-6 shrink-0 overflow-x-auto md:overflow-hidden bg-base z-10 transition-all duration-500 ${isLocked ? 'opacity-30' : 'opacity-100'}`}>
          <nav className="flex flex-row md:flex-col gap-6 md:gap-4 text-[13px] text-textSec items-center md:items-start whitespace-nowrap">
            ...
            {isLocked ? (
              <span className="md:pl-4 text-textMuted cursor-not-allowed opacity-50 block">Dashboard</span>
            ) : (
              <Link href="/" className="md:pl-4 hover:text-accentCyan cursor-pointer transition-all block">Dashboard</Link>
            )}
            
            <div className="text-accentCyan flex items-center gap-2 font-medium">
              <span className="text-[10px] hidden md:block">◉</span> Academics
            </div>

            {isLocked ? (
              <span className="md:pl-4 text-textMuted cursor-not-allowed opacity-50 block">Research</span>
            ) : (
              <Link href="/research" className="md:pl-4 hover:text-accentCyan cursor-pointer transition-all block">Research</Link>
            )}

            {isLocked ? (
              <span className="md:pl-4 text-textMuted cursor-not-allowed opacity-50 block">Fitness & Diet</span>
            ) : (
              <Link href="/fitness" className="md:pl-4 hover:text-accentCyan cursor-pointer transition-all block">Fitness & Diet</Link>
            )}

            {isLocked ? (
              <span className="md:pl-4 text-textMuted cursor-not-allowed opacity-50 block">Archive</span>
            ) : (
              <Link href="/archive" className="md:pl-4 hover:text-accentCyan cursor-pointer transition-all block">Archive</Link>
            )}

            {isLocked ? (
              <span className="md:pl-4 text-textMuted cursor-not-allowed opacity-50 block">IELTS</span>
            ) : (
              <Link href="/ielts" className="md:pl-4 hover:text-accentCyan cursor-pointer transition-all block">IELTS</Link>
            )}

            {isLocked ? (
              <span className="md:pl-4 text-textMuted cursor-not-allowed opacity-50 hidden md:block">Tools & Links</span>
            ) : (
              <Link href="/tools" className="md:pl-4 hover:text-accentCyan cursor-pointer transition-all hidden md:block">Tools & Links</Link>
            )}

            {/* IDENTITY NODE ACTIVATED */}
            {isLocked ? (
              <span className="md:pl-4 text-textMuted cursor-not-allowed opacity-50 hidden md:block">Identity</span>
            ) : (
              <Link href="/identity" className="md:pl-4 hover:text-accentCyan cursor-pointer transition-all hidden md:block">Identity</Link>
            )}
          </nav>
        ...
        <main className={`flex-1 flex flex-col gap-6 p-4 md:p-6 overflow-y-auto overflow-x-hidden transition-all duration-700 ${focus.mode === 'focused' ? 'bg-surface shadow-[inset_0_0_100px_rgba(6,182,212,0.03)]' : 'bg-base'}`}>
          ...
          <div className="flex justify-between items-end shrink-0">
            <div>
              <h1 className="font-barlow text-[28px] text-textPri font-bold uppercase tracking-wide leading-none">Preclinical Hub</h1>
              <p className="text-[13px] text-textSec mt-1">Year 3 CMU Medical Curriculum & Intelligence Vault</p>
            </div>
            
            <button 
              onClick={toggleFocusProtocol}
              className={`text-[11px] font-mono px-4 py-1.5 rounded transition-all duration-300 flex items-center gap-2 font-bold tracking-widest
          ...
          {/* NOTEBOOK LM PORTAL */}
          <div className={`relative border rounded-lg p-6 overflow-hidden flex flex-col gap-4 shrink-0 transition-all duration-500 ${focus.mode === 'focused' ? 'bg-base border-accentCyan shadow-[0_0_30px_rgba(6,182,212,0.15)] scale-[1.01]' : 'bg-surface border-accentCyan/40 shadow-[0_0_20px_rgba(6,182,212,0.05)]'}`}>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center relative z-10 gap-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded bg-base border border-borderline flex items-center justify-center text-xl">📔</div>
                <div>
                  <h2 className="font-barlow font-bold text-[16px] text-textPri uppercase tracking-wide">NotebookLM Intelligence Vault</h2>
                  <p className="text-[11px] text-textSec">AI-Powered Synthesis & Source Grounding</p>
                </div>
              </div>
              <a 
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2 relative z-10">
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer" 
                className={`group bg-base border rounded p-3 transition-all flex flex-col gap-1 relative overflow-hidden ${focus.mode === 'focused' ? 'border-accentCyan shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'border-accentCyan/50 hover:border-accentCyan'}`}
              >
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer" 
                className="group bg-base border border-borderline border-dashed rounded p-3 hover:border-accentCyan/50 transition-all flex flex-col gap-1 items-center justify-center opacity-60 hover:opacity-100"
              >

...
export default function FitnessHub() {
  ...
  // V3 Metrics State - Now includes discrete values for calculations
  const [metrics, setMetrics] = useState({ 
    sleepHours: 7, 
    sleepMinutes: 30, 
    weight: 72.4, 
    height: 175, // cm for BMI
    water: 2.1 
  });
  ...
  useEffect(() => {
    ...
    // Bumped to V3 for new metric schemas
    const savedMeals = localStorage.getItem('vestrippn_meals_v3');
    const savedMetrics = localStorage.getItem('vestrippn_metrics_v3');
    const savedCycle = localStorage.getItem('vestrippn_cycle_v3');
  ...
  useEffect(() => {
    if (isMounted) {
      localStorage.setItem('vestrippn_meals_v3', JSON.stringify({ date: new Date().toLocaleDateString(), meals }));
      localStorage.setItem('vestrippn_metrics_v3', JSON.stringify(metrics));
      localStorage.setItem('vestrippn_cycle_v3', activeCycle.toString());
    }

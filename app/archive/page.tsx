    ...
    <>
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden bg-base">
        ...
        <aside className="w-full md:w-[220px] border-b md:border-b-0 md:border-r border-borderline flex flex-row md:flex-col justify-between p-4 md:p-6 shrink-0 overflow-x-auto md:overflow-hidden bg-base z-10">
          <nav className="flex flex-row md:flex-col gap-6 md:gap-4 text-[13px] text-textSec items-center md:items-start whitespace-nowrap">
            ...
            <div className="text-accentCyan cursor-pointer transition-all flex items-center gap-2 font-medium">
              <span className="text-[10px] hidden md:block">◉</span> Archive
            </div>
        ...
        <main className="flex-1 flex flex-col gap-6 p-4 md:p-6 overflow-y-auto overflow-x-hidden bg-base">
          ...
          {/* HEADER SECTION (The only part that follows Day/Night theme) */}
          <div className="flex justify-between items-end shrink-0 mb-2">
            <div>
              <h1 className="font-barlow text-[28px] text-textPri font-bold uppercase tracking-wide leading-none">Telemetry Archive</h1>
              <p className="text-[13px] text-textSec mt-1">Global Database Matrix & Identity Nodes</p>
            </div>
            <div className="flex gap-3">
              <div className="text-[11px] font-mono text-[#22c55e] border border-[#22c55e]/30 bg-[#22c55e]/10 px-3 py-1 rounded flex items-center gap-2">
          ...
          <div className="group flex-1 bg-[#0a0a0a] border-4 border-[#1a1a1a] rounded-xl shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden relative cursor-crosshair">
            ...
            <div className="h-4 bg-[#111] border-b border-[#222] flex justify-center items-center gap-1.5 px-4 pt-1 shrink-0">
              {[...Array(10)].map((_, i) => (
                <div key={`g-${i}`} className="w-6 h-2 rounded-sm bg-[#222] transition-all duration-75 group-hover:bg-[#22c55e] group-hover:shadow-[0_0_8px_rgba(34,197,94,0.8)]" style={{ transitionDelay: `${i * 30}ms` }}></div>
              ))}
              {[...Array(4)].map((_, i) => (
                <div key={`a-${i}`} className="w-6 h-2 rounded-sm bg-[#222] transition-all duration-75 group-hover:bg-[#f59e0b] group-hover:shadow-[0_0_8px_rgba(245,158,11,0.8)]" style={{ transitionDelay: `${(i + 10) * 30}ms` }}></div>
              ))}
              {[...Array(3)].map((_, i) => (
                <div key={`r-${i}`} className="w-6 h-2 rounded-sm bg-[#222] transition-all duration-75 group-hover:bg-[#ef4444] group-hover:shadow-[0_0_8px_rgba(239,68,68,0.8)]" style={{ transitionDelay: `${(i + 14) * 30}ms` }}></div>
              ))}
              {[...Array(2)].map((_, i) => (
                <div key={`b-${i}`} className="w-6 h-2 rounded-sm bg-[#222] transition-all duration-75 group-hover:bg-[#3b82f6] group-hover:shadow-[0_0_8px_rgba(59,130,246,0.8)]" style={{ transitionDelay: `${(i + 17) * 30}ms` }}></div>
              ))}
            </div>

            {/* Steering Wheel Dashboard Data - Absolute Colors Only */}
            <div className="flex justify-between items-center bg-[#111] px-6 py-3 border-b border-[#333] shrink-0">
              <div className="flex gap-6 font-mono">
                <div>
                  <div className="text-[10px] text-gray-500 mb-0.5 uppercase">Speed</div>
                  <div className="text-[18px] text-[#06b6d4]/50 group-hover:text-[#06b6d4] group-hover:drop-shadow-[0_0_8px_rgba(6,182,212,0.8)] transition-all duration-500 font-bold leading-none">
                    314<span className="text-[10px] text-gray-400 ml-1">KPH</span>
                  </div>
                </div>
                <div>
                  <div className="text-[10px] text-gray-500 mb-0.5 uppercase">BBW</div>
                  <div className="text-[18px] text-white/50 group-hover:text-white transition-all duration-500 font-bold leading-none">
                    56.5<span className="text-[10px] text-gray-400 ml-1">%</span>
                  </div>
                </div>
              </div>
              
              <div className="flex flex-col items-center justify-center">
                <div className="text-[10px] text-gray-500 mb-1 uppercase">Gear</div>
                <div className="text-4xl text-[#22c55e]/30 group-hover:text-[#22c55e] group-hover:drop-shadow-[0_0_12px_rgba(34,197,94,0.8)] transition-all duration-500 font-bold leading-none">
                  8
                </div>
              </div>
              ...
              <div className="flex gap-6 font-mono text-right">
                <div>
                  <div className="text-[10px] text-gray-500 mb-0.5 uppercase">Strat</div>
                  <div className="text-[18px] text-white/50 group-hover:text-white transition-all duration-500 font-bold leading-none">MODE 2</div>
                </div>
                <div>
                  <div className="text-[10px] text-gray-500 mb-0.5 uppercase">SOC</div>
                  <div className="text-[18px] text-[#f59e0b]/50 group-hover:text-[#f59e0b] group-hover:drop-shadow-[0_0_8px_rgba(245,158,11,0.8)] transition-all duration-500 font-bold leading-none">
                    82<span className="text-[10px] text-gray-400 ml-1">%</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Action Bar */}
            <div className="bg-[#1a1a1a] px-4 py-2 flex justify-between items-center z-10 border-b border-[#2a2a2a] shrink-0">
              <span className="text-[11px] font-mono text-gray-400">TARGET: MASTER DIRECTORY</span>
              <a 
                href="https://linktr.ee/shankusu.studygram" 
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-[#06b6d4]/10 hover:bg-[#06b6d4]/20 border border-[#06b6d4]/50 text-[#06b6d4] px-4 py-1.5 rounded text-[10px] font-bold tracking-widest uppercase transition-colors"
              >
                OG Linktree ↗
              </a>
            </div>

            {/* NATIVE DIRECTORY - EVERY TEXT ELEMENT HARDCODED */}
            <div className="flex-1 w-full bg-[#0a0a0a] overflow-y-auto p-6 flex flex-col gap-8 custom-scrollbar relative z-20">
...
function ArchiveLink({ title, url, icon }: { title: string, url: string, icon: string }) {
  return (
    <a 
      ...
      target="_blank" 
      rel="noopener noreferrer"
      className="group/link bg-[#111] border border-[#222] p-3 rounded-lg flex items-center justify-between hover:border-[#06b6d4] hover:shadow-[0_0_15px_rgba(6,182,212,0.1)] transition-all overflow-hidden relative z-30"
    >
      ...
      <div className="flex items-center gap-3 pl-2">
        ...
        <span className="text-[13px] text-white font-medium truncate">{title}</span>
      </div>
      <span className="text-gray-500 group-hover/link:text-[#06b6d4] transition-colors opacity-50 group-hover/link:opacity-100 font-mono text-[10px]">
        ACCESS ↗
      </span>
    </a>
  );
